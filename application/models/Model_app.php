...
class Model_app extends CI_model{
    ...
    public function produk_perkategori($id_reseller,$id_produk_perusahaan,$id_kategori_produk,$limit){
        return $this->db->query("SELECT * FROM tb_produk WHERE id_kategori_produk='$id_kategori_produk' ORDER BY id_produk DESC LIMIT $limit ");
    }
    ...
    function cari_produk($kata){
        ...
        $cari = "SELECT * FROM tb_produk WHERE ";
    ...
    function jual($id){
        return $this->db->query("SELECT sum(a.jumlah) as jual FROM tb_penjualan_detail a JOIN tb_penjualan b ON a.id_penjualan=b.id_penjualan where a.id_produk='$id' AND b.proses='1'");
    }

    function beli($id){
        return $this->db->query("SELECT sum(a.jumlah_pesan) as beli FROM tb_pembelian_detail a where a.id_produk='$id'");
    }

    function jual_umum($produk){
        return $this->db->query("SELECT sum(jumlah) as jual FROM `tb_penjualan` a JOIN tb_penjualan_detail b ON a.id_penjualan=b.id_penjualan where b.id_produk='$produk' AND a.proses!='0'");
    }

    function beli_umum($produk){
        return $this->db->query("SELECT sum(jumlah_pesan) as beli FROM `tb_pembelian_detail` where id_produk='$produk'");
    }
    ...
    function orders_report($id){
        return $this->db->query("SELECT * FROM `tb_penjualan` a where a.id_pembeli='$id' ORDER BY a.id_penjualan DESC");
    }

    function konfirmasi_bayar(){
        return $this->db->query("SELECT b.kode_transaksi, a.*, c.* FROM `tb_konfirmasi` a JOIN tb_penjualan b ON a.id_penjualan=b.id_penjualan JOIN tb_rekening c ON a.id_rekening=c.id_rekening ORDER BY a.id_konfirmasi_pembayaran DESC");
    }

    function orders_report_all(){
        return $this->db->query("SELECT * FROM `tb_penjualan` a ORDER BY a.id_penjualan DESC");
    }

    function orders_report_home($limit){
        return $this->db->query("SELECT * FROM `tb_penjualan` a ORDER BY a.id_penjualan DESC LIMIT $limit");
    }
